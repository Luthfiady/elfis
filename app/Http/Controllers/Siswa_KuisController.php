...
// -------------------------------------------------------- KUIS --------------------------------------------------------
	...
	public function kuis_get_list() {
		if(session('id_group') == 1) {
			...
			$cek_data_soal = DB::select('select id_group_kuis from nilai_kuis where nis="'.session('id_user').'"');
			...
			if ($cek_data_soal!=null) {
				...
				foreach ($cek_data_soal as $cek => $gets) {
					...
					$p_id = $gets['id_group_kuis'];
					$jml_data = count($cek_data_soal);
					$l = $jml_data;

					for($c=1; $c<=$l; $c++){					 
						$and_data .= 'and a.id_group_kuis<>"'.$p_id.'" ';
					}
				}
			...
			$data_rows = DB::select('select a.*,b.nama_materi,c.nama from group_kuis a join materi b join guru c where a.id_materi=b.id_materi and b.nik=c.nik and (kuis_mulai<=ADDDATE("'.date('Y-m-d').'",7) and kuis_selesai>="'.date('Y-m-d').'") '.$and_data.' '.$sql_ext);
			...
			$data_kuis = DB::select('select a.*,b.nama_materi,c.nama from group_kuis a join materi b join guru c where a.id_materi=b.id_materi and b.nik=c.nik and (kuis_mulai<=ADDDATE("'.date('Y-m-d').'",7) and kuis_selesai>="'.date('Y-m-d').'") '.$and_data.' '.$sql_ext.' ORDER BY a.kuis_mulai ASC LIMIT '.$per_page.' OFFSET '.$offset);
			...
			if ($data_kuis != true) {
				...
				$result .= '<tr>';
				$result .= '<td colspan="8">Tidak ada soal kuis</td>';
				$result .= '</tr>';
				$result .= '</tbody>';
				$result .= '</table>';
				...
				foreach ($data_kuis as $row => $list) {
					...
					$tgl_kuis_mulai = $list['kuis_mulai'];

					if ($tgl_kuis_mulai > date('Y-m-d')) {
						$result .= '<td class="kolom-tengah"><a class="btn btn-xs btn-warning" href="kuis/'.$list['nama_group_kuis'].'/'.$list['id'].'" disabled>
									<span class="glyphicon glyphicon-new-window"></span></a>
								</td>';
					} else {
						$result .= '<td class="kolom-tengah"><a class="btn btn-xs btn-warning" href="kuis/'.$list['nama_group_kuis'].'/'.$list['id'].'">
									<span class="glyphicon glyphicon-new-window"></span></a>
								</td>';
					}
	...
	public function kuis_soal($nama_group_kuis, $id) {
		if(session('id_group') == 1) {
			...
			if(isset($_SESSION["durasi_kuis".$user.$id])) {
				$session_durasi = time() - $_SESSION["durasi_kuis".$user.$id];
			} else {
				$_SESSION["durasi_kuis".$user.$id] = time();
				$session_durasi = 0;
			}

			return view('view_siswa/kuis/soal')->with('nama_group_kuis', $nama_group_kuis)->with('id', $id)->with('session_durasi', $session_durasi);
		}
		else {
			return redirect('login');
		}
	...
	public function kuis_get_id() {
		if(session('id_group') == 1) {
			...
			$get_id = DB::select('select * from group_kuis where id='.$id);
		...
		if(session('id_group') == 1) {
			...
			if ($data_kuis == null) {
				...
				foreach ($data_kuis as $row => $list) {
					...
					$radio_show = DB::select('select jawaban from param_radio where id_jwb_user="radio_'.$id_jwb_user.'"');
					...
					$result .= '<input type="hidden" name="id_group_kuis" value="'.$list['id_group_kuis'].'">';
	...
	public function AddParamJawaban() {
		if(session('id_group') == 1) {
			...
			if(isset($_POST)){
				...
				$id_group_kuis = $_POST['id_group_kuis'];
				...
				for($i=1; $i<=$a; $i++){
					...
					if(isset($_POST['id_soal_'.$a])){
						...
						$cek_data = DB::select('select id_jwb_user from param_radio where id_jwb_user="'.$id_jwb_user.'"');

						if (isset($_POST[$id_jwb_user])) {
							$jawaban = $_POST[$id_jwb_user];

							if ($cek_data == null) {
								$insert_param_jwb = DB::insert('insert into param_radio values("", "'.$id_group_kuis.'", "'.$id_soal.'", "'.$jawaban.'", "'.$id_jwb_user.'", "'.date('Y-m-d H:i:s').'", "'.session('id_user').'")');
							} else {
								$update_param_jwb = DB::update('update param_radio set jawaban="'.$jawaban.'" where id_jwb_user="'.$id_jwb_user.'"');
							}
						} else {
	...
	public function kuis_get_nilai() {
		...
		if(session('id_group') == 1) {
			...
			$result .= '<legend> <h3 class="legend"> Hasil Kuis </h3> </legend>';
			...
			$result .= '<div class="form-group">
							<div class="col-sm-12" style="text-align:center;">
								<a href="../../kuis" type="submit" class="btn btn-primary btn-sm">Tutup</a>
							</div>
					    </div>';
			$result .= '</div>';
			$result .= '<div class="col-md-4"></div>';
			...
			$insert_nilai = DB::insert('insert into nilai_kuis values("", "'.$id_group_kuis.'", "'.session('id_user').'", "'.$total_nilai.'", "'.date('Y-m-d H:i:s').'")');
